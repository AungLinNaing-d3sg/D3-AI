...
export interface SceneState {
  ...
  /** Drives the three orbiting "identity" rings (see
   * components/three/IdentityConstellation.tsx) that stand in for the
   * company's Data / Dynamics / Digital pillars — visible primarily during
   * the About chapter, with a faint echo during the Company/Team chapter to
   * tie the "people" story back to the same identity. */
  identity: {
    opacity: number;
  };
  light: {
...
export const sceneKeyframes: SceneState[] = [
  ...
  // The identity rings (Data / Dynamics / Digital) reach full opacity here —
  // this is their one dedicated "chapter".
  {
  ...
  // 6. Company/Team — technology settles, warms into a human tone. Identity
  // rings return at low opacity, tying the "people" story back to the same
  // Data/Dynamics/Digital identity introduced in the About chapter.
  {
...
export function resetSceneState() {
  ...
  Object.assign(sceneState.identity, rest.identity);
